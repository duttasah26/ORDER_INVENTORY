...
export function useCreateOrder() {
  ...
  return useMutation({
    mutationFn: async ({ customerId, storeId, items }) => {
      ...
      // The mock server has no custom POST route for /orders, so this falls
      // through to json-server's default create, which auto-assigns `id`
      // but does NOT populate `order_id` - even though every seed order row
      // has `id === order_id`, and every lookup used elsewhere (cancel,
      // byIdOrStore, useOrder) keys off `order_id`. Backfill it so a
      // freshly created order is immediately viewable/cancellable like any
      // other order.
      if (createdOrder && createdOrder.order_id === undefined) {
        await axios.put(endpoints.orders.update(), {
          id: createdOrder.id,
...
// GET-as-mutation quirk, documented in endpoints.js / orderApi.js.
